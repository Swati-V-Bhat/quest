...
import { useExperiment } from '@/hooks/useABTest'; // Import A/B Testing Hook
import questService from '@/lib/questService';
import { createPost } from '@/lib/postService';
import { Map, Calendar, ArrowLeft, Clock, MapPin as MapPinIcon, Filter, Edit3, Save, Copy, Plus, Trash2, GripVertical, ChevronDown, ChevronUp, Share2, Send, X, Plane, Train, Bus, Car, Ship, Edit2, Image as ImageIcon, Sparkles } from 'lucide-react';
...
const QuestClient = () => {
  ...
  // A/B Test Hook
  const { variant, isCarousel, isStory, isVideo } = useExperiment('QUEST_CREATION_FLOW');
  ...
  // Initialize isEditMode from URL but keep it as state to allow toggling
  const [isEditMode, setIsEditMode] = useState(searchParams.get('edit') === 'true');

  // Update isEditMode if URL changes (optional, but good for navigation)
  useEffect(() => {
    ...
    <div className="min-h-screen bg-gray-950 text-white pb-20 lg:pb-0">
            ...
            <div className="flex items-center gap-2">
              {isOwner ? (
                ...
                <>
                  ...
                  {/* A/B Tested AI Generation Button */}
                  {isCarousel && (
                    <button
                      onClick={() => setShowCarouselModal(true)}
                      className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-orange-500 to-pink-500 text-white rounded-lg hover:from-orange-600 hover:to-pink-600 transition-all font-medium text-sm shadow-lg shadow-orange-500/20"
                    >
                      <ImageIcon size={18} />
                      Generate Carousel
                    </button>
                  ...
                  {isStory && (
                    <button
                      ...
                      className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-purple-500 to-pink-500 text-white rounded-lg hover:from-purple-600 hover:to-pink-600 transition-all font-medium text-sm shadow-lg shadow-purple-500/20"
                    >
                      <Sparkles size={18} />
                      Generate Story
                    </button>
                  ...
                  {isVideo && (
                    <button
                      ...
                      className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-blue-500 to-cyan-500 text-white rounded-lg hover:from-blue-600 hover:to-cyan-600 transition-all font-medium text-sm shadow-lg shadow-blue-500/20"
                    >
                      <Video size={18} />
                      Generate Video
                    </button>
                  ...
                  {/* Fallback / Debug: Show all if no variant (shouldn't happen with our logic, but good for dev) */}
                  {!isCarousel && !isStory && !isVideo && (
                    <div className="text-xs text-gray-500">Loading AI options...</div>
                  )}
      ...
      <div className="hidden lg:flex max-w-[100vw] mx-auto h-[calc(100vh-65px)] overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-700 scrollbar-track-gray-900 scroll-smooth">
          <article className="max-w-4xl mx-auto p-4 md:p-6 pr-8 md:pr-12">
            {displayQuest.itinerary?.days?.map((day: any, dayIndex: number) => (
              <section key={dayIndex} id={`day-${day.day}`} className="mb-8">
                <button
                  onClick={() => toggleDayCollapse(dayIndex)}
    ...
    <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl overflow-hidden shadow-xl border border-gray-700 h-full cursor-pointer hover:scale-105 transition-transform">
      {activity.media?.[0]?.url && (
        <div className="relative h-32 overflow-hidden">
          <Image src={activity.media[0].url} alt={activity.title || 'Activity Image'} fill className="object-cover" />
